...
### 1.2 Component Relationship (High-Level Architecture)

```mermaid
graph LR
    User((User)) <--> Webview[Webview UI / React]
    Webview <--> EH[Extension Host / TypeScript]
    EH <--> LLM[LLM / Cloud]
    EH --> HE[(Hook Engine / Middleware)]
    HE -.-> FS[File System / .orchestration]
```
...
- **Dispatcher:** `src/core/assistant-message/presentAssistantMessage.ts` (switch on `block.name` → `tool.handle()`).
- **Prompt Logic:** `src/core/prompts/system.ts` and `src/core/prompts/sections/`.
- **Primary Tools:** `src/core/tools/WriteToFileTool.ts`, `src/core/tools/ExecuteCommandTool.ts`.
...
- **Intent Gatekeeper:** Blocks mutating tools unless an intent id is set, returning `INTENT PROTOCOL VIOLATION`.
- **Scope Enforcement:** Blocks reads/writes outside the `owned_scope` defined in the intent, returning `SCOPE VIOLATION`.
